...
class TestPay2goInvoice < MiniTest::Test
  ...
  def test_raise_invalid_mode_error
    assert_raises Pay2goInvoice::InvalidMode do
      Pay2goInvoice::Client.new(mode: :invalid)
    end
  end

  def test_raise_missing_option_error
    assert_raises Pay2goInvoice::MissingOption do
      Pay2goInvoice::Client.new
    end
  end

  def test_raise_missing_parameter_error
    assert_raises Pay2goInvoice::MissingParameter do
      @client.generate_mpg_params
    end
  end

  def test_raise_unsupported_type_error
    assert_raises Pay2goInvoice::UnsupportedType do
